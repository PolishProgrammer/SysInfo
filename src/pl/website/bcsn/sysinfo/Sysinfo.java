...
public class Sysinfo extends JavaPlugin {
	...
	public void onEnable() {
		...
		localeFile = new File(dataFolder, "locale.yml"); // locale.yml
		...
		for (String s : locales_names) {
			getLogger().info("Added " + s + " locale to list");
			copy(getResource("locale-" + s + ".yml"), new File(dataFolder,
					"locale-" + s + ".yml"));
		}
		...
		aThr.runTaskTimer(this, 20L, (long) config.getInt("ram-check-interval")*20); 
		getLogger().info("Starting RAM check task!");
	...
	private void msgSysinfo(CommandSender sender) {
		...
		senderMSG("cores: "+InfoGatherer.getInfo(infoType.MACHINE_CPU_CORES), sender);
	}
